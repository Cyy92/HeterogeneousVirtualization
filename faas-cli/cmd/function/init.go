...
var initCmd = &cobra.Command{
  ...
  faas-cli function init <FUNCTION_NAME> -r <RUNTIME> [-f <APPEND_EXISTING_YAML_FILE>] [-g <FX_GATEWAY_ADDRESS>]`,
	Short: "Prepare a FaaS function",
	Long: `
...
`,
	Example: `  faas-cli function init echo --runtime go
  faas-cli function init read-write -f ./config.yml -r python3
  faas-cli function init read-write --config ./config.yml --runtime java --gateway localhost:31113
  `,
	PreRunE: preRunInit,
	RunE:    runInit,
}
...
func preRunInit(cmd *cobra.Command, args []string) error {
	...
	if len(gateway) > 0 {
		fxServices.FaaS.FxGatewayURL = gateway
	}
